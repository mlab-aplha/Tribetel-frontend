...
const apiService = {
    ...
    async verifyPayment(paymentIntentId: string): Promise<ApiResponse<PaymentResponse>> {
        try {
            ...
            // Map Stripe status to your simplified status
    ...
    async refundPayment(bookingId: string): Promise<ApiResponse<PaymentResponse>> {
        try {
            ...
            // Map refund status to your simplified status
            const refundStatus: PaymentResponse['status'] = response.data.status === 'succeeded' ? 'succeeded' : 'failed';
